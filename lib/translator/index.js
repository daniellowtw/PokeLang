...
var read = function(data, comments) {
  ...
  while (tokens.length > 0) {
    ...
    if (isString(token)) {
      ...
      var new_tokens = createStringTokens(token);

      tokens = new_tokens.concat(tokens);
...
var createStringTokens = function(token) {

  var new_tokens = ['['];

  for (var i = 1; i < token.length - 1; i++) {

    new_tokens.push('' + token[i].charCodeAt());
...
var doOneMove = function(first) {
  ...
  switchEnemyPoke(random_pokemon);
