...
var createStates = function() {
  ...
  createState('SELFTURN', function() {
    ...
    matchLine(line, ['$POKEMON', 'uses', '$MOVE']); 

    checkSelfPokemon(line[0]);

    // check $POKEMON knows $MOVE
...
var peekWord = function(actual, expected) {
  ...
  return compareWord(actual, expected, true);
...
var checkSelfPokemon = function(pokemon) {
  ...
  if (battle.selfPokemon !== pokemon) {

    throw error.wrongPokemon(battle.selfPokemon, pokemon);
